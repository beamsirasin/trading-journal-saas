...
export function SegmentedControl<T extends string>({
  ...
  return (
    <fieldset className={cn('min-w-0', className)}>
      ...
      <div className="bg-background border-border inline-flex flex-wrap gap-1 rounded-lg border p-1">
        {options.map((option) => {
                ...
                {checked ? (
                  prefersReducedMotion ? (
                    <span
                      ...
                      aria-hidden="true"
                      className="bg-surface-raised border-border shadow-control absolute inset-0 rounded-md border"
                    />
                  ) : (
                    <motion.span
