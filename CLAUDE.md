...
- **Actual Risk is a separate Risk Discipline observation.** It never redefines canonical Actual R (§4). **Amended 2026-09-23 (contract decision 53):** it is recorded in **Entry Context & Evidence** (stage 4) as an entry-time execution fact, never in Plan & Risk, which holds the plan — Risk at Entry, Stop Method, Target, Exit Plan and price context. **Amended 2026-09-23 (contract decision 54):** **Planned Risk** (`trades.planned_risk_state`, migration 0030) is Unanswered / Defined Risk (with `planned_risk_minor` > 0) / No Defined Risk; Record Open requires the decision, not a monetary risk, and a No Defined Risk Trade produces no Actual R, Trader R or RR and is not asked Actual Risk at all. Stop Method (`trades.planned_stop_method`, migration 0029) is **withdrawn from capture** by the same decision: no UI, no new writes, nothing inferred from stored values, column and migration retained. Neither is ever inferred from an SL price. **Amended 2026-09-23 (contract decision 52):** it starts **Unanswered** in both recording moments and becomes an answer only when the trader states Matched or Different; Matched is never inferred from the absence of a Different answer, and a Save from the unanswered state stores no answer. Trades saved before the amendment keep their recorded `matched` — a real confirmation under the interaction contract that applied then, never backfilled.
