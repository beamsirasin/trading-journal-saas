...
- **One Risk per Trade; Actual Risk is retired from capture (contract decision 56, 2026-09-25).** Risk & target records the amount the trader defines as 1R — **Planned Risk** (`trades.planned_risk_state`, migration 0030) is Unanswered / Defined Risk (with `planned_risk_minor` > 0) / No Defined Risk, and Record Open requires the decision, not a monetary risk — **amended 2026-09-26 (contract decision 59): Required = required for completion, not for saving.** The decision (with Target, and the Exit Plan under No Fixed Target) is Required for completion; an Open Trade may be saved with it Unanswered (migration 0033), and Close Existing Trade's Final Close is refused (`final_close_incomplete`) until every Required item of Steps 1–5 is answered, asking missing plan answers on the close page itself. Record Closed and Record Open never gate Save on Required items; the shared model is `src/lib/trades/requirements.ts`. Trader R = Final Net P&L ÷ Risk and System R = System Result ÷ Risk; No Defined Risk produces no Trader R, System R or RR. No recording flow asks Actual Risk and no Save writes `actual_risk_answer` / `actual_initial_risk_minor`; values recorded before the retirement (including decisions 52–53's Matched / Different answers) are historical only — shown read-only where present, never backfilled, never an R or analytics input. Stop Method (`trades.planned_stop_method`, migration 0029) is likewise withdrawn from capture (decision 54). Neither Risk nor anything else is ever inferred from an SL price.
- **After Trade records a factual Plan Outcome (contract decision 55).** Record Closed asks the stages in canonical order — Trade, Risk & target, Strategy & setup, Entry context, Trader result, After trade — and After Trade begins with a System Result: what the original plan would have produced (`trades.plan_outcome`, migration 0032). Target first / risk first derive money and R from the plan (`src/lib/calc/plan-outcome.ts`); a rule-based Exit Plan takes a stated amount; No Defined Risk asks nothing. It is capture evidence, never `trade_system_assessments`, and carries no Net / Gross claim.
...
- **Final Net P&L is authoritative** for a closed Trade. Exit subtotals are supporting history, adopted only explicitly, and a Complete-history discrepancy is non-blocking (§11). **Amended 2026-09-25 (contract decision 57) for new Record Closed capture:** Step 5 records how the Trade closed — _Closed all at once_ (one All remaining exit) or _Closed in parts_ (exit legs) — and the Final Net P&L is derived from that close (the full close's P&L, or the sum of the exits once an All remaining exit or 100% of stated shares proves the position closed; otherwise none, only a labelled running figure). There is no separate Final Net P&L input, no "Use recorded exits" and no discrepancy notice there; the Save sends the derived figure as adopted from the exits and the service re-checks it (`final_pnl_source = 'exit_history'`, no schema change). **Decision 58:** a close in parts may instead be recorded as _I only know the final result_ — a stated total (`finalPnlStatedTotal`, stored `manual_total`), no exits invented — and the service refuses any Record Closed Final Net P&L without exactly one of those two claims (`final_pnl_source_invalid`). Close Existing Open Trade and Trades already saved with a stated Final Net P&L keep the rule above.
...
- derives `trader_outcome` from R with the ±0.05R band, or from P&L sign, enforced by `trades_status_consistency_check`, on every close except Save Closed Trade (After Trade), which since migration 0023 stores the trader's own Win / BE / Loss or Unanswered (`trader_outcome_selected_at`), and the contract Final Close (`recordContractExit`, migration 0027), which does the same for an Open contract Trade through the canonical Stage 5 page (`/app/trades/close`, "Record partial exit" / "Close trade" on the Trade's Execution panel) — the only close path for a contract Trade since the legacy exit / close / exit-correction actions began refusing it (`contract_close_required`); a contract row closed through them before that keeps its derived outcome, and canonical analytics treat its R as legacy evidence;
