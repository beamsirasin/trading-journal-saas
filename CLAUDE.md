...
>
> **Status:** Phases 00, 00b, 01, 02, 02.1 and 3A complete and merged to `main` (authentication, tenant-isolated workspaces, registration hardening, onboarding and the first trading account). Phase 3B (trading account management and the active-account switcher) implemented on branch `phase/03b-trading-account-management`, not yet merged. A verified, onboarded user can now view, create, edit, archive, and restore trading accounts at `/app/accounts`, and switch the active account from the authenticated app shell; creation is idempotent (`trading_accounts.mutation_key`), archiving the active account atomically falls back to the oldest remaining usable account, and the last usable account in a workspace can never be archived. Strategies, trades, analytics calculations, subscriptions, payments, and account-count entitlement limits remain fixture-driven, absent, or explicitly deferred to Phase 3C/04.
...
| A13 | Account-count entitlement limits (1/5/10, A3) are deliberately NOT enforced yet — Phases 3A and 3B ship onboarding and full account management (create/edit/archive/restore/switch) with no gate; the gate arrives with billing (Phase 04) or Phase 3C, whichever lands first                                                                                                                                   | Phase 04   |
