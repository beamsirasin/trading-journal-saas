...
- **Actual Risk is a separate Risk Discipline observation.** It never redefines canonical Actual R (§4). **Amended 2026-09-23 (contract decision 52):** it starts **Unanswered** in both recording moments and becomes an answer only when the trader states Matched or Different; Matched is never inferred from the absence of a Different answer, and a Save from the unanswered state stores no answer. Trades saved before the amendment keep their recorded `matched` — a real confirmation under the interaction contract that applied then, never backfilled.
