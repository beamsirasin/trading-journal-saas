...
>
> **Status:** Phases 00, 00b, 01, 02, 02.1, 3A and 3B complete and merged to `main` (authentication, tenant-isolated workspaces, registration hardening, onboarding, the first trading account, full trading-account management, and the active-account switcher). Phase 3C (trial entitlements and paid-plan account limits) implemented on branch `phase/03c-trial-entitlements`, not yet merged. A workspace's trial starts atomically with onboarding completion and is a fixed **1-account, 7-day, full-feature trial** — never derived from any paid plan's limit. Three paid plans are locked: **Starter** (1 account, ฿149/$5 per month), **Trader** (5 accounts, ฿299/$9 per month), **Professional** (15 accounts, ฿499/$15 per month) — every paid plan shares the identical feature set; the account limit is the only entitlement difference between them. Prices are tax-exclusive (VAT collection is disabled — the business is not yet VAT-registered). `createTradingAccount`/`restoreTradingAccount` enforce the effective limit inside their existing locked transactions; archive/edit/switch are never gated. Strategies, trades, analytics calculations, and real payment-provider integration remain fixture-driven, absent, or explicitly deferred to Phase 04.
> **Last updated:** 2026-08-04 (corrected — an earlier draft of this branch shipped placeholder plan keys `starter`/`pro`/`elite` with limits 1/3/10 and a trial derived from the highest paid plan's limit; both are superseded by the locked decision above)
>
...
| A3  | **Locked in Phase 3C** — three paid plans, gating exclusively on active trading-account count, identical feature set: Starter (1 account, ฿149/$5 per month), Trader (5 accounts, ฿299/$9 per month), Professional (15 accounts, ฿499/$15 per month). Prices are tax-exclusive; VAT collection is disabled (not yet VAT-registered). Superseded the Phase 01 provisional 1/3/10 starter/pro/elite draft. Registry: `src/config/plans.ts`.                                                                                                                                                                                                                                                   | Phase 3C ✓ |
...
| A17 | No online payment provider is connected in Phase 3C — every plan card's call to action is a disabled "Coming soon," never a real purchase, activation, or entitlement mutation from the client. Real billing (Stripe/Omise/etc.), VAT calculation, invoicing, and `current_period_ends_at` maintenance are Phase 04's job.                                                                                                                                                                                                                                                                                                                                                                  | Phase 04   |
