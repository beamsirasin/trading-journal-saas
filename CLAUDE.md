...
>
> **Status:** Phases 00, 00b, 01, 02 and 02.1 complete and merged to `main` (authentication, tenant-isolated workspaces, registration hardening, verification redelivery). Phase 3A (onboarding and the first trading account) implemented on branch `phase/03-onboarding-trading-accounts`, not yet merged. A verified user now completes a short onboarding wizard that creates a real, tenant-scoped trading account (`trading_accounts`), becomes their active account (`user_preferences.active_trading_account_id`), and marks their workspace's onboarding complete (`workspaces.onboarding_completed_at`); `/app` shows a real, honest empty-state dashboard for it. Strategies, trades, analytics calculations, subscriptions, payments, and account-count entitlement limits remain fixture-driven, absent, or explicitly deferred to Phase 3B/04.
...
| A13 | Account-count entitlement limits (1/5/10, A3) are deliberately NOT enforced yet — Phase 3A ships onboarding and one account with no gate; the gate arrives with billing (Phase 04) or Phase 3B, whichever lands first                                                              | Phase 04   |
