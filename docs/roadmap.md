...
| [11](phases/PHASE-11-admin.md)                   | SaaS Administration                                        | 🚧 **In progress — 11C delivered** | 11B: `platform_admins`/`admin_audit_log`/entitlement `source`/`platform_vat_configuration` foundation. 11C: EN-only `/admin` Overview dashboard (total users/workspaces, effective subscription state, plan distribution, 30-UTC-day new-user/Trade activity) — read-only, no oversight pages, mutations, or VAT wiring yet |
