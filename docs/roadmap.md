...
| [10](phases/PHASE-10-settings.md)                | Settings                                                   | 🚧 **In progress — 10D delivered** | Real Profile, Preferences, Workspace/Billing integration, and versioned owner-only Data Export delivered; Security remains                                                                                                                                                                                                  |
| [11](phases/PHASE-11-admin.md)                   | SaaS Administration                                        | 🚧 **In progress — 11D delivered** | 11B: `platform_admins`/`admin_audit_log`/entitlement `source`/`platform_vat_configuration` foundation. 11C: EN-only `/admin` Overview dashboard (total users/workspaces, effective subscription state, plan distribution, 30-UTC-day new-user/Trade activity). 11D: read-only, searchable/paginated `/admin/users` and `/admin/workspaces` list + detail pages, privacy-limited by contract — still no mutations or VAT wiring |
