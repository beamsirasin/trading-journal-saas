...
1. **Six canonical stages.** Each stage owns its questions, wherever a flow presents it:
   ...
   | **2 — Plan & Risk**              | Risk at Entry — the intended 1R; Actual Risk — the risk execution actually carried at entry, in the mode's own form (§3.4, §12.4); Target (Target Profit, TP price); Exit Plan; price levels — Entry, SL, size — folded as context |
...
| Uploaded chart images as before-entry evidence                | A server-side pending-upload slice | Only the TradingView link is offered. Today an image is written to storage before any Trade exists, the only record of it is the browser, and nothing reclaims it: Discard, sign-out, draft retention cleanup or a closed tab would each orphan it. Upload waits for a server-side record of every pending upload, claimed atomically by Save and reclaimed when unclaimed (§5.6, §5.11). |
...
- **Recording lifecycle (§20), 2026-09-22:**
  ...
  - Record Closed Trade (After Trade) still presents five steps — Trade, Result, Plan, Context and
    ...
    decision 51 places it. Its Context step holds the shared Setup & Checklist stage (Strategy and
    Setup as launcher rows, every condition answered in place), then the shared Entry Context &
    Evidence stage (Confidence, Entry Emotion in a focused editor, why this trade, timeframe,
    session, entry notes and the chart link as before-entry evidence), then Post-Trade Emotion,
    which waits there until After-Trade Context is built. The Save step holds no answers: it is
    the read-back and Save.
