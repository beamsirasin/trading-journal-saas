...
> **Approved Add Trade target (contract §3):** price is context only and never a denominator for canonical P&L, Actual R, System R or System Result. A plausible inconsistency such as a Long trade with the stop above entry is therefore a non-blocking data-quality notice, not a Save-blocking error; only malformed price input is an error. Price-derived context such as a distance or pips may be explored later but never becomes calculation authority. The rejection above is current implementation until migration.
...
**Save Closed Trade (After Trade, migration 0023).** A contract row saved closed stores `actual_r = actualR(Final Net P&L, Risk at Entry)` only when both are known; otherwise `actual_r` is NULL with a stated reason (needs Final Net P&L, needs Risk at Entry, or both) — never 0R and never a figure derived from prices or exit legs. Its `trader_outcome` is the trader's own Win / BE / Loss (`trader_outcome_selected_at` set) or NULL for Unanswered; it is never classified from R or P&L sign, and a sign-contradicting choice carries only a quiet notice. A later Risk at Entry correction re-measures Actual R but keeps the selected (or Unanswered) outcome (`recalculatedTraderOutcome`).
...
winRate      = wins / answeredCount           (break-evens excluded from the
                                               numerator, included in the denominator;
                                               unanswered outcomes in neither)
...
**Unanswered outcomes are excluded from every outcome metric, never counted as losses** (Add Trade contract §24, §25). An `OutcomeRecord` carries `outcome: null` for an unanswered outcome; `winRate`, `averageWinR`, `averageLossR` and `payoffRatio` count answered outcomes only, and a non-empty sample with none answered returns `no_outcomes_answered` — never 0%. `outcomeCounts` is published as `null` in that case, so no surface can print "0 wins, 0 losses".
...
**Canonical analytics populations (implemented 2026-09-17).** Canonical analytics never silently combine legacy and Add Trade contract v1 evidence (Add Trade contract §25, §28). `src/server/dal/canonical-analytics-population.ts` is the one SQL definition, composed by every Dashboard, Analytics, Calendar, Day Review and insight read; `src/lib/analytics/canonical-population.ts` holds the shared vocabulary. It is a different question from `entryContextAnalyticsEligible` (was entry context captured before the close?), and behavioural reads apply both.

- **Canonical Actual R** — a closed Trade with `actual_r` that is a contract row (`recording_contract = 'add_trade_v1'`) with a Money result and a stated Final Net P&L (`final_pnl_source IS NOT NULL`, written by Save Closed Trade and the contract Final Close), so its Actual R is `Final Net P&L / Risk at Entry`. Legacy Actual R (historical Actual Risk as 1R) and Price-mode R are excluded, and so is the R of a contract row closed by the legacy live close (2026-09-22): its net P&L was derived from exit legs, never stated, so it counts as legacy coverage. A row with no Actual R passes this clause, so its selected outcome still reaches the outcome figures. **A final exit time is not required (2026-09-19):** After Trade may save a closed Trade without one (contract §13), and it stays in every R total. Only reads that need a time use `canonicalDatedActualConditions()` — the Calendar, Day Review, insight pillars and the comparison candidates — and a date range excludes an undated Trade because `exited_at >= start` is never true for NULL.
- **Canonical Trader population** — `canonicalTraderConditions()`: a closed contract Money row with an Actual R **or** a trader-selected outcome. Each figure reads only the rows carrying its evidence (`composeTraderAnalytics`): R figures the rows with R; Win Rate and outcome counts every row, counting selected outcomes only, so a selected outcome on a Trade with no Risk at Entry counts in Win Rate and never in an R figure (contract §25); the equity curve and drawdown only R rows with an exit time. Strategy, Setup and context breakdowns follow the same split (`summarizeAxis`). `PerformanceAnalyticsModel` reports `sampleCount` (R), `outcomeSampleCount` and `undatedCount`.
- **Canonical Trader Outcome** — only a trader-selected outcome on a contract row (`trader_outcome_selected_at IS NOT NULL`), written by Save Closed Trade (After Trade, migration 0023) and by the contract Final Close (`recordContractExit`, migration 0027 — the Stage 5 "Close trade" page). `canonicalTraderOutcome()` projects every other row's outcome as `null` — a derived outcome from the legacy live close or from any legacy row never counts. When no Trade in scope has a selected outcome, Win Rate, Avg Win R, Avg Loss R and Payoff report `no_outcomes_answered`. A Trade's Actual R still counts in every R metric whether or not its outcome is answered.
- **Canonical System Result** — none yet. The trader-confirmed System Assessment is not implemented, so every resolved System result (on a legacy or a contract row) is legacy System R. The System population, Population C (paired / Execution Gap / System Edge Captured), and the System and Gap calendars admit nothing, and no row-level read (Recent Trades, Day Review, insight actual Trades, comparison candidates) carries a System R beside a canonical Actual R.
- **Net P&L** is money and has no legacy form: it reads every closed Trade in scope (`selectClosedTradeMoneyRecords`), with or without a final exit time, so it agrees with the Trade list and Risk Performance. A closed Trade with no Final Net P&L (legacy Price mode, or an After Trade save without one) makes it honestly `incomplete`.
- **Coverage** — `selectLegacyAnalyticsCoverage` counts the excluded evidence per axis on that axis's own date gate (`excludedActualCount` on `exited_at`, `excludedSystemCount` on `system_exited_at`), plus `undatedClosedCount` (closed Trades in scope with no final exit time) and `dateRangeActive`: inside a date range undated Trades are in no figure, otherwise they are in every total and only time-ordered figures leave them out — the note says which. Published as `AnalyticsSnapshot.legacyCoverage` and `DashboardPageData.coverage.legacy` and disclosed in one sentence on the Dashboard, Analytics and the Trades summary. Excluded rows are absent from samples, never zero-R samples or losses.
- Rule and Mistake counts are not R or outcome metrics and keep their closed-Trade population.

**Population A — Trader eligible (the R predicate).** `isTraderEligible` requires `status = 'closed'`, no soft deletion and `actual_r`; `exited_at` is required only by time-ordered figures (`hasExitTime`). `isTraderOutcomeEligible` (closed, not deleted) is the outcome figures' own population. A Trader Outcome is **not** required: an unanswered outcome is a missing observation (contract §24), so it leaves outcome metrics only. Strategy, Setup, and System resolution do not gate global eligibility (classification matters only when its corresponding filter is selected). The date axis and deterministic ordering are `exited_at`, then Trade ID. The canonical SQL population above narrows this to contract rows.
...
- **Trader Outcome.** On a contract row it is the trader's selected outcome, or `unavailable` with reason `outcome_not_selected` — a stored `trader_outcome` without a selection was derived by the pre-contract Final Close, and Win / BE / Loss is the trader's own answer (contract §12). The Result column says NOT ANSWERED, distinct from a legacy row's NO RESULT. A legacy row keeps its classification, and every surface that shows it — the Trades table, the mobile list, the Trade Details header and Execution tab — carries the `Legacy` marker beside it (2026-09-19). A legacy row's Actual R carries the same marker in the table, the mobile list and the header, and the header's System / Actual / Gap comparison for a legacy row is labelled legacy evidence.
...
| `no_outcomes_answered`                                                                                                                                                                                                    | Add Trade | `winRate`, `averageWinR`, `averageLossR`, `payoffRatio` — a non-empty sample with no answered Trader Outcome; never 0% and never losses                             |
