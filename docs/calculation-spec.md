...
- **Canonical Trader Outcome** — only a trader-selected outcome on a contract row (`trader_outcome_selected_at IS NOT NULL`), written by Save Closed Trade (After Trade, migration 0023) and by the contract Final Close (`recordContractExit`, migration 0027 — no UI calls it yet). `canonicalTraderOutcome()` projects every other row's outcome as `null` — a derived outcome from the legacy live close or from any legacy row never counts. When no Trade in scope has a selected outcome, Win Rate, Avg Win R, Avg Loss R and Payoff report `no_outcomes_answered`. A Trade's Actual R still counts in every R metric whether or not its outcome is answered.
