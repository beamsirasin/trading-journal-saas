...
- **Canonical Trader Outcome** — none yet. Every stored `trader_outcome`, contract rows included, is derived from R on close; Final Close does not record the trader's choice. Canonical reads therefore project `traderOutcome: null`, and Win Rate, Avg Win R, Avg Loss R and Payoff report `no_outcomes_answered`. A Trade's Actual R still counts in every R metric.
