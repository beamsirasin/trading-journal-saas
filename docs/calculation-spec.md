...
**Status:** Phase 07 is officially complete (07A audit, 07B schema, 07C per-Trade engine, 07D aggregate/attribution engine, 07E full regression and closeout). Phase 07C implemented the per-trade calculation core — `src/lib/calc/{types,decimal,risk,trade}.ts` — against the `trades` schema Phase 07B made real (`drizzle/0008_trade_domain_and_discipline.sql`). Phase 07D implemented the aggregate/attribution/equity engine on top of it — `src/lib/calc/{aggregate,attribution,equity}.ts`: Total/Average R, Expectancy, Win Rate, average Win/Loss, Payoff Ratio, Profit Factor, the cumulative-R equity curve, Maximum Drawdown, paired System-vs-Trader edge leakage, execution efficiency, and objective Rule adherence. §4/§5/§6 below reflect that implemented contract, reconfirmed unchanged during 07E closeout. **This document is the one canonical formula document** — no separate `docs/formulas.md` was created; that role is fulfilled here. **Deliberately NOT implemented:** Discipline Score (no approved formula exists), the weighted mistake-penalty formula, mistake-cost ranking, attribution of edge leakage to individual mistake types, and verdict sample-size thresholds — all deferred to a later phase's explicit, evidence-backed decision; see §5's own subsection. No service, DAL, Server Action, UI, database write path, date-bucketed reporting, or SQL aggregation exists yet (Phase 08/09's job) — the entire engine is pure, importable, and testable without a database connection, environment variables, or the Next.js runtime.
