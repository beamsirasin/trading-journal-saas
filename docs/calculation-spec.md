...
- **Canonical Actual R** — a closed Trade with `actual_r` and `exited_at` that is a contract row (`recording_contract = 'add_trade_v1'`) with a Money result, so its Actual R is `Final Net P&L / Risk at Entry`. Legacy Actual R (historical Actual Risk as 1R) and Price-mode R are excluded.
...
- **Net P&L** is money and has no legacy form: it reads every closed Trade in scope (`selectClosedTradeMoneyRecords`), so it agrees with the Trade list and Risk Performance. A closed Trade with no Final Net P&L (legacy Price mode) makes it honestly `incomplete`.
- **Coverage** — `selectLegacyAnalyticsCoverage` counts the excluded evidence per axis on that axis's own date gate (`excludedActualCount` on `exited_at`, `excludedSystemCount` on `system_exited_at`), published as `AnalyticsSnapshot.legacyCoverage` and `DashboardPageData.coverage.legacy` and disclosed in one sentence on the Dashboard, Analytics and the Trades summary. Excluded rows are absent from samples, never zero-R samples or losses.
