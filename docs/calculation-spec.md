...
- **Canonical Trader Outcome** — only a trader-selected outcome on a contract row (`trader_outcome_selected_at IS NOT NULL`), which today is written only by Save Closed Trade (After Trade, migration 0023). `canonicalTraderOutcome()` projects every other row's outcome as `null` — a derived outcome from a live close (Final Close does not yet record the trader's choice) or from any legacy row never counts. When no Trade in scope has a selected outcome, Win Rate, Avg Win R, Avg Loss R and Payoff report `no_outcomes_answered`. A Trade's Actual R still counts in every R metric whether or not its outcome is answered.
