...
2. **`src/server/auth/dal.ts`** — `getOptionalSession()` calls `auth.api.getSession()`, which re-verifies against the database (or the 60-second cookie cache Better Auth manages internally as a read optimization — never trusted alone for an authorization decision). This is the actual security boundary, run again inside every protected layout, page, and server action.
...
- The 60-second cookie cache is an internal Better Auth read optimization, not a second source of truth this codebase has to reason about — the cache either agrees with the database or gets bypassed on the next verification window; nothing here trusts it for a security decision.

**Negative / accepted**

- Every protected request does at least one database read (unless served from Better Auth's internal cache window) — an accepted cost given the alternative (stateless JWT) either can't revoke promptly or ends up needing its own database-backed denylist anyway, at which point it is not actually stateless.
...
| No cookie cache at all (`cookieCache.enabled: false`)                                     | Every single request would hit Postgres for session verification; the 60-second cache meaningfully reduces load for a chatty client (React re-renders, layout re-fetches) without weakening the actual security check, since the DAL layer still re-verifies for every sensitive operation regardless of what the cache returned.                                                |
