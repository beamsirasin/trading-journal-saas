...
| Execution Gap (formerly Edge Leakage, renamed/sign-corrected in Phase 13H) | `Execution Gap`                                 | Kept as a loanword, unchanged across EN/TH — the retired `edgeLeakageR`/"Edge Leakage" no longer appears anywhere in product copy.                                                       |
| Discipline Score                                                           | `คะแนนวินัย`                                    | Exception: this one **is** translated — "discipline" and "score" both have exact, unambiguous Thai equivalents traders already use, unlike "Expectancy" or "Execution Gap".              |
