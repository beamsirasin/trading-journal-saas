...
| Demo fixture content (trade symbols, strategy names like "London breakout", account nicknames like "Prop challenge — 100K") | Fixture data, not UI chrome — the same category as a real trader's own account name, which this product would never translate. |
...
| Edge Leakage     | `ขอบที่รั่วไหล (Edge Leakage)`                  | Thai gloss given alongside the English term on first/prominent use (e.g. the attribution section heading), English alone elsewhere once introduced.                                      |
