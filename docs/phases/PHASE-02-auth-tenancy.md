...
### Fixed during this phase's CI hardening

Two pre-existing Phase 1 defects, first surfaced by this phase wiring Playwright into CI (`/en/app` had no auth guard before, so nothing previously forced these assertions to run against real content in an automated way) — confirmed present on `main` at `b8a6aa5`, unrelated to authentication or tenancy, but fixed rather than deferred since they were the only remaining CI failures:

- **Header overflow at 320px** (`app-shell.spec.ts` — "has no horizontal overflow at 320px"): the app shell's mobile header row — menu trigger, wordmark, language switcher, theme toggle, account menu, all fixed-size 44px touch targets plus the wordmark — has a real minimum content width (~368px) wider than a 320px viewport. Fixed by hiding the wordmark's text label (keeping its icon badge) below 400px in that specific header instance (`Brand`'s new `compact` prop) — no touch target shrinks, brand identity is not lost, and the text returns once there is room.
- **Sub-pixel touch-target measurement** (`app-shell.spec.ts` — "keeps mobile shell controls at least 44px in each touch dimension"): the drawer's close button is authored at exactly `size-11` (44px); `getBoundingClientRect()` returned `43.99999237060547` for it — floating-point layout rounding, not a shrunk control. The test now rounds each measurement to the nearest CSS pixel before comparing, which still fails on any real regression.
