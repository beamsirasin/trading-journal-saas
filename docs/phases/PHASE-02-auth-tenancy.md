...
- **Two pre-existing Phase 1 responsive-layout failures**, first surfaced by this phase's work (wiring Playwright into CI — `/en/app` had no auth guard before, so nothing previously forced these assertions to run against real content in an automated way): `e2e/app-shell.spec.ts` — "has no horizontal overflow at 320px" and "keeps mobile shell controls at least 44px in each touch dimension" (off by ~0.00001px, almost certainly sub-pixel rendering precision). Both assertions and the page content they check predate Phase 2 (confirmed identical on `main` at `b8a6aa5`) and are CSS/layout issues unrelated to authentication or tenancy — out of scope for this phase to fix. Left as a follow-up.
