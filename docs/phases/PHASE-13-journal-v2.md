...
This is the **opposite sign** of the pre-13H `edgeLeakageR = systemR − actualR` runtime (`src/lib/calc/attribution.ts:60-71`, whose doc comment said positive means the Trader captured less R). Before this freeze, that legacy convention also appeared in `CLAUDE.md` §6; 13A resolved the authority conflict there with an explicit supersession/runtime note. The old runtime convention was not preserved as the product contract merely because it existed — **13H flipped and renamed it**: `edgeLeakageR`/`pairedEdgeLeakageR` no longer exist; `src/lib/calc/attribution.ts` now exports `executionGapR(actualR, systemR)`, `pairedExecutionGapR`, and `averageExecutionGapR` (the new primary aggregate, §6 below), and every call site enumerated in the blast-radius table was updated together with the rename — see the 13H implementation record in §16.
...
- Phase 13G redesigns Trade Detail into six grouped sections — Overview, Plan, **Entry Snapshot** (new: Setup Conditions, Confidence, Emotions, Entry Reason), Actual Execution, System Result, and **Discipline & Review** (Execution Rules, Mistakes, Post-Trade Review) — and reads back Setup Conditions on Detail for the first time via a derived three-state disclosure (`recorded` / `not_recorded` / `not_configured`), computed read-only from two already-existing sources (the Trade's own `trade_setup_condition_checks` rows, and a count of `setup_conditions` for its pinned Setup Version) — no new column. Trade List gains Planned R as the baseline figure, a batched partial-close Realized R + remaining-% read (one grouped `trade_exits` query per page, reusing `composeRealizedActual`), and an optional batched Setup Condition adherence chip (`§9`'s prescribed grouped-count-by-Trade-ID shape) — never per-row queries. Both Detail and List now truthfully disclose when a Trade's live Strategy/Setup/Trading Account is archived, without ever rewriting the pinned historical label. The legacy-signed `edgeLeakageR` (§1) was audited and confirmed absent from every Trade Detail/List component; 13H's sign flip has no call site to touch there. `TradeReflectionEditor` was split into `TradeEmotionsEditor` (Entry Snapshot) and `TradeReviewNotesEditor` (Discipline & Review) — same underlying actions, no domain-behavior change.
- Phase 13H flips and renames the Execution Gap globally: `src/lib/calc/attribution.ts`'s `edgeLeakageR`/`pairedEdgeLeakageR` are retired and replaced by `executionGapR(actualR, systemR)`/`pairedExecutionGapR`/`averageExecutionGapR` (the new primary aggregate, `AVG(actualR − systemR)` over the paired population, §6); `ComparisonAnalyticsModel.edgeLeakageR` is renamed `executionGapR` with `averageExecutionGapR` added alongside it; every consuming surface (Comparison panel, Dashboard, marketing `attribution`/`productPreview`, demo fixtures/dashboard, EN/TH copy) was updated together, and the full blast-radius table in §1 has no remaining legacy-signed call site. Phase 13H also ships the first Setup Adherence, Condition, Confidence, and Emotion analytics — new DAL selectors in `src/server/dal/analytics.ts` (`getSetupAdherenceAnalyticsRecords`, `getConditionAnalyticsRecords`, `getConfidenceAnalyticsRecords`, `getEmotionAnalyticsRecords`), a new `src/lib/calc/setup-adherence.ts` (per-Trade adherence, the primary `averageSetupAdherence` and secondary `conditionsMetRate`, and the five fixed §10 buckets), and new compose functions in `src/lib/analytics/metrics.ts`, rendered as four new cards under `/app/analytics`'s new **Setup Quality** and **Psychology** sections. **Scope decision (documented, not a hidden gap):** all four new dimensions are implemented against the Trader/Actual-eligible population only, per the frozen contract's own repeated framing of a System-side breakdown as optional/secondary ("and/or", "where useful") for these specifically — not against System eligibility or as a paired/System comparison. Every dimension respects Trader eligibility (closed, nondeleted, `actual_r`/`trader_outcome`/`exited_at` all present) and the Trader date axis (`exited_at`); a still-open Trade with recorded Conditions/Confidence/Emotions is excluded, proven by integration test. Setup Adherence/Conditions Met Rate exclude "not recorded" and "not configured" Trades structurally (inner join on `trade_setup_condition_checks` — a Trade with zero rows never enters either aggregate). Condition-level analytics group by `(setupId, conditionKey)` — proven stable across a simulated Setup Version copy-on-write in both the DAL and service integration suites — and display the most-recent-by-exit snapshot label. Confidence excludes `NULL` and includes `0` as a real recorded value. Emotion analytics give one row per (Trade, Emotion) link, so a multi-Emotion Trade legitimately appears in more than one group, and a recorded-zero-selection Trade contributes to none — both proven by integration test. No migration (0010–0014 unchanged, no `0015`) — every field already existed from 13B–13G.
