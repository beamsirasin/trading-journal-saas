...
- `src/server/auth/dal.ts` — server-only, re-verifies against the database on every call (`getOptionalSession`/`requireSession`, via `auth.api.getSession`). This is where a forged, expired, or revoked cookie is actually rejected. Every protected layout, page, and server action calls into this module, never `auth.api.getSession` directly.
