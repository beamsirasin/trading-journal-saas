...
> **Authority:** this document is the **implementation reference** for the visual system — concrete tokens, component implementations and measured decisions. Visual philosophy, hierarchy, composition and visual rules live in [`DESIGN.md`](../DESIGN.md) at the repository root; where the two disagree on visual direction for new work, `DESIGN.md` wins and this document is updated when that work lands. Both sit below approved [Product Contracts](product-contracts/README.md) (product semantics), [`UX_RULES.md`](UX_RULES.md) (interaction and behaviour) and `CLAUDE.md` engineering constraints. A visual choice here never overrides, or simplifies away, a semantic or behavioural rule from those documents; if a visual treatment cannot represent one clearly, report the conflict.
...
**Every fact on the Dashboard has exactly one owner, and the active Account's name belongs to the toolbar.** The toolbar's Account control prints it as its trigger label, permanently and at every breakpoint. The context strip beneath the page title used to print the same string ~60 vertical pixels below it — the same fact twice inside one viewport, with the copy that could not be clicked coming second. The strip stays, and now carries only what the trigger does **not**: account mode, base currency, and starting balance, introduced by the one visible "Active account" caption. It is still read-only — no control, no chevron, no affordance that would invite a click — because a second account selector is exactly what a reader must never find here. Nothing left the accessibility tree: the region names itself with `aria-label`, and the two chips keep their `sr-only` captions.

**The shell header's own account switcher yields to a page that has one.** That switcher is the application-wide default and the only way to change the active Account on Trades, Strategies, Accounts, Analytics, Settings, Plan and Billing — it is not going anywhere. It stands down on exactly the routes listed in `ROUTES_WITH_OWN_ACCOUNT_CONTROL` (`components/shell/constants.ts`), which today means the Dashboard alone, because that page's toolbar already carries an Account control beside Date Range and Filters. Two switchers for one piece of state, sixty pixels apart and behind two different gestures, is a contradiction a reader has to resolve rather than a convenience — so the page-level one wins where it exists. `ShellFrame` reads `usePathname` to decide, because the header is rendered by the shared `(app)` layout: a page below that layout cannot reach up and turn one of its controls off, and threading a flag down through every route's server layout would put a Dashboard detail into the signature of every page in the product. The match is **exact**, never a prefix — `/app/trades` and `/app/accounts` are not the Dashboard. Nothing replaces the hidden control: the utilities cluster is `ml-auto` with a `gap`, so it stays flush right whether it holds one child or two, and suppressing the switcher shifts nothing.

**The toolbar's three controls animate as one control, from one seam.** Date Range, Filters and Account all render through `ToolbarTrigger` and `ToolbarDisclosure`, so the motion is defined twice — once for the trigger, once for the panel — rather than three times. The trigger gets a 150ms transition over colour and transform only (never `transition-all`, which drags box metrics and the focus ring onto the same clock), a `0.98` press deflection that costs no layout, and a chevron that rotates 180° off the button's own Radix `data-state` through a named group, so open/closed has a single source of truth. The desktop panel reuses the existing `menu-content-in/out` shape — 0.95 scale plus a fade, 180ms in and 120ms out — anchored to `--radix-popover-content-transform-origin` so it grows out of the trigger's own corner. That animation is opted into with `data-motion="toolbar"`, mirroring `SheetContent`'s `data-motion="shell"`, rather than applied to `PopoverContent` generally: the same primitive serves the KPI band's definition popovers, and giving every popover in the product motion is a separate decision. The mobile branch is a `Sheet` and already animates. Under `prefers-reduced-motion` the press deflection is suppressed, the trigger's transition becomes instant, and the panel's scale is replaced by a plain fade on the already-shortened menu clock — every state still readable, nothing travelling.
...
**The Basic KPI band answers five questions, in this order.** Net P&L (how much money), **Total R** (how much in R), **Win Rate** (how often), **Avg Planned RR** (what was planned), **Avg R / Trade** (what a Trade returns on average). Profit Factor, Day Win % and Avg Win / Loss stood in the last three slots and are retired from the band: between them they were three second readings of the same win/loss shape, and a beginner reading five figures should not have to hold three of them together to learn anything the first two did not already say. All three remain canonical, are still composed onto `DashboardPageData['basic']`, and are read on Analytics — presentation decides what is shown, the payload decides what is available.

**Four of the five are ACTUAL; exactly one is PLAN, and it says so.** Net P&L, Total R, Win Rate and Avg R / Trade all read the frozen Trader population (Population A). Avg Planned RR reads the persisted `planned_r` of those same Trades and never touches Actual execution, System resolution, realized P&L or an exit price. That distinction is the product's central one (CLAUDE.md §1), so the card's ⓘ states outright that the figure comes from the plan made _before_ entry, and its popover names the population it averaged — a Trade recorded with no planned target carries no ratio and is excluded rather than counted as a zero plan. Total R and Avg R / Trade are the ACTUAL axis specifically: the same `trader.totalR` the System vs Trader card labels "Actual Total R", never the System axis and never the paired subset.

**Sign colour is spent on the three signed outcomes only.** Net P&L, Total R and Avg R / Trade are results whose direction is the whole content of the number, so they carry `--positive` / `--negative`. Win Rate and Avg Planned RR stay `--foreground` at every value: a win rate is a level, not a verdict, and a planned ratio is an intention. Net P&L keeps the row's lead treatment (`text-kpi-hero`) and its Trade-count line, so it still leads on typography rather than on colour.

**Avg Win / Loss is `payoffRatio`, and the name is deliberate.** It is `lib/calc`'s `payoffRatio` — `averageWinR / abs(averageLossR)` with break-even excluded from both averages — computed for both axes by the same `composePerformanceAxis`. Each side reads only its own frozen population (B for System, A for Trader), so the two ratios legitimately differ. The label is never "Average RR", "Risk:Reward" or "Avg R": risk-reward describes _planned_ SL/TP geometry and Avg R is the mean R per Trade, while this is realized average winner magnitude against realized average loser magnitude. An absent denominator is stated in words (`no_wins` / `no_losses`) — never `0x`, never `∞`, never a misleading `100%`.
...
| Card           | Shape                | Encodes                                                              |
| -------------- | -------------------- | -------------------------------------------------------------------- |
| Total R        | cumulative sparkline | the Trader axis's own `equityCurve` — its last vertex IS the figure  |
| Win Rate       | ring (donut)         | W / BE / L Trade counts — three counts that partition the population |
| Avg Planned RR | split track          | `1 / (1 + RR)`, the published ratio restated as risk against reward  |
| Avg R / Trade  | zero-centred bar     | direction first, magnitude second, against a fixed ±1R domain        |

**Net P&L gets none.** It is a single signed money total and no per-Trade or per-day money series is published for that population; a sparkline would have to be invented or borrowed from the paired Execution-Gap population, which is a different Trade universe. That card is carried by typography, which is the right answer when the data is not there. Note that this is exactly why Total R _can_ have one: R has a published Population A series and money does not.

**The Avg R / Trade scale is fixed, and that is what makes it readable.** The visual domain is ±1R, documented as `AVG_R_INDICATOR_DOMAIN_R`, so the same deflection means the same thing on every account, range and filter; a dataset-relative scale would silently redraw a `+0.05R` account as a full bar. Values outside the domain clamp — the bar then reads "off the scale, in this direction" and the figure above it stays the authoritative value. Clamping never touches the number.

**The sparkline thins by dropping vertices, never by averaging them.** A 400-Trade population has roughly seven vertices per visible pixel at 56px wide, so it keeps at most 32 — always including the first and the last, so every point drawn is a real cumulative total and the curve still starts and ends where the population does. Zero is always inside its vertical domain: scaling to the series' own extremes alone would fit a curve that never left profit into the same box as one that never left loss.

**The shapes differ on purpose.** Four cards in a row drawing four rings would read as one widget printed four times; a ring, a line, a split track and a deflection keep four different questions legible at a glance. They stay one family through a shared size band (ring 40px, sparkline 24×56/80, split track 12px over 56/80, deflection 12px over 56/80), one palette, one trigger treatment and one popover.

**The indicator is a button where it has something to reveal.** The Win Rate composition (`Wins 27`, never `27W`) and the Avg Planned RR sentence open from a click, a tap, or Enter; a hover-only tooltip would have made that data unreachable on touch and by keyboard. The two whose figure already says everything the drawing does stay a plain picture rather than a button that opens an empty panel. The drawing itself is `aria-hidden`; where there is a button it carries the name and the popover carries the figures, so a reader who sees no colour loses nothing.

**Avg Planned RR's popover states its own denominator, and has to.** It is the one card in the band whose population can be smaller than the row's: a Trade recorded with no planned target carries no ratio and is excluded rather than counted as a zero plan. A reader comparing "66 Trades" under Net P&L with this average has exactly one place to find out it came from 58 of them.
...
**The indicator must read as data, not as decoration.** The benchmark's KPI cards carry donuts, a partial ring and a ratio bar with real mass against their 120px card; an indicator small enough to be mistaken for an icon is worse than none, because it implies a reading it is too small to give. These sizes moved up with the card's padding and should move with it again if the card's height ever changes.
