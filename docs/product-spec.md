...
> **Add Trade authority:** the approved [Add Trade Product Contract](product-contracts/add-trade.md) (v1, 2026-09-14) is the product source of truth for recording, closing, reviewing and assessing Trades. Where older wording here differs, the contract wins; statements that still describe current implementation pending migration are labelled. Interaction behaviour for these flows is governed by [`UX_RULES.md`](UX_RULES.md), which sits below the contract and above this document for behaviour.
...
All performance is normalised to **R** — the **Risk at Entry**, the risk the trader intended to accept, which is the common 1R baseline for both Actual R and System R. A trade risking $100 to make $250 is `+2.5R`.

R is what makes system and trader results comparable. Because both use the same Risk at Entry, a trader who takes more or less risk than intended shows up in Actual R and in Risk Discipline (Actual Risk compared with Risk at Entry), rather than being hidden by a second denominator. The current implementation still divides Actual R by actual risk until migration.

### The outcome matrix

The System Result and the trader-selected Trader Outcome are **independent stored fields**. The System Result is never inferred from actual profit or price, and Trader Outcome is never derived from P&L or R.

|                            | Trader win                                                          | Trader loss                                                    |
| -------------------------- | ------------------------------------------------------------------- | -------------------------------------------------------------- |
| **Positive System result** | Followed a good signal                                              | **The valuable cell** — a working system, damaged by execution |
| **Negative System result** | Made money by breaking the rules — dangerous, reinforces bad habits | Followed a bad signal; the strategy is the problem             |
...
Mitigations: never prefill or infer the System Result from the actual result or price (it is always trader-confirmed), keep the wording neutral so recording a mistake does not feel like a confession, and be candid in the product that output quality follows input honesty. This is inherent to a manual journal and is not solved by more UI.
...
| 4   | **Locked in Phase 07C:** `BREAK_EVEN_TOLERANCE_R` is `0.05R`, a global Calculation Engine Version 1 constant (`src/config/trade-calc.ts`) — identical for every Workspace and every Trading Account, not per-account configuration. See CLAUDE.md A1. **Superseded for Trader Outcome** by the approved Add Trade contract: Trader Outcome is trader-selected; the constant remains in the current implementation until migration. | Phase 07 ✓ |
