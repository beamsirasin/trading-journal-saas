...
**Status:** Living document. Phase 03, Phase 04 — Billing & Checkout, Phase 05 — Onboarding & Trading Accounts, Phase 06 — Strategies & Versions, and Phase 07 — Trade Model & Calculation Engine are officially complete; Phase 08 — Trade Journal is next. Authentication, tenant-isolated workspaces, onboarding, trading accounts, the 7-day trial, active-account entitlements, monthly-plan checkout, immutable billing snapshots, conditional VAT presentation, and real workspace-owned Strategies with nested Setups, versioned Rules, and a full management UI are real, behind a mock payment provider not reachable by ordinary production traffic. Phase 07 delivered the real Trade/mistake/Rule-check schema and a pure, tested calculation engine (`src/lib/calc/`) — Planned/Actual/System R, outcome classification, and aggregate/attribution/equity metrics all compute correctly against plain data, but no UI, service, or write path exists yet, so no trade can be recorded and no metric below is visible to a user yet — that is Phase 08's (entry) and Phase 09's (dashboard) job.
