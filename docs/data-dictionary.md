...
| Context                   | `symbol`, `direction` (`long`\|`short`), `timeframe`, `session`, `confirmation_notes`, `confidence` (exactly one of `0`/`25`/`50`/`75`/`100`, or `NULL` — Founder-UAT Confidence redesign; migration 0010 updated in place, superseding both an earlier uncommitted continuous 0–100 draft and, before that, a 1–5 rating), `tradingview_url`, `notes` |                                                                                                                                                                                                                                                                                                                                                       |
...
| **Actual**                | `actual_entry`, `actual_initial_stop`, `actual_exit`, `actual_position_size` (informational only), `entered_at`, `exited_at`                                                                                                                                                                                                                           | What the trader did — always Price, never Money (Actual execution is never optional-representation the way the Plan is)                                                                                                                                                                                                                               |
