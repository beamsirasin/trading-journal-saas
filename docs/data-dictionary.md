...
**System status is a third, independent state machine** from `status` and from the outcome classifications — CLAUDE.md §1's outcome matrix requires this. `pending` has no terminal System snapshot. `no_trade` fixes `system_exit_reason = setup_invalidated` and clears both resolution inputs. `resolved` requires `system_resolution_kind`, `system_exited_at`, reason, `system_r`, and outcome atomically. `price_exit` requires Price Plan geometry plus `system_exit_price` and forbids `system_gross_r_input`. The four `money_*` kinds require a Money-only Plan, forbid a fabricated exit price, and require the canonical gross input; Target additionally requires Planned R. `system_cost_r` is zero for pending/no-trade and non-negative for resolved. The database OR-shaped consistency constraint makes incompatible or stale combinations unrepresentable.
