...
| Envelope            | `version` (currently `1`), `activeMode` (`at_entry` / `after_trade`), `mutationKey` (UUID; the Save idempotency key, so a retry after failure or reload cannot duplicate a Trade), `updatedAt`, `atEntry` and `afterTrade` (each mode's own section, kept while inactive), `lastCarried` (the shared values last carried, so a later switch carries only what changed)                                                                                                                                                 |
| Versioning          | An envelope of any other `version`, or one that fails its schema, is **unrecoverable**: nothing is filled in from it, the trader is told, and it is replaced only when new work is written. No older shape is reinterpreted                                                                                                                                                                                                                                                                                            |
| Carry between modes | Only values with identical meaning in both forms: Account, Symbol, Direction, an explicit (`trader`) Entry time, a Money Risk at Entry, Strategy, Setup, confidence, emotions, reason, chart URL, notes, timeframe, session. An untouched "now" Entry time, the implicit Actual Risk match and an inherited Strategy-default Exit Plan never carry. Condition answers, Exit Plan and Actual Risk stay in the At Entry section because the legacy After Trade form cannot represent them without changing their meaning |
