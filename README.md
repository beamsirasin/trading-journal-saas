...
**Phase 03, Phase 04 — Billing & Checkout, Phase 05 — Onboarding & Trading Accounts, Phase 06 — Strategies & Versions, and Phase 07 — Trade Model & Calculation Engine are officially complete and merged to `main`. Phase 08 — Trade Journal is the next implementation phase.**

Authentication, tenant-isolated personal workspaces, onboarding, trading-account management, the active-account switcher, the 7-day trial, server-enforced active-account entitlements, monthly-plan checkout, immutable billing snapshots, and conditional VAT presentation are real. Phase 05 reviewed and polished the onboarding/account-management UX (archived-account detail parity, archive-not-delete clarity, restore-limit guidance, clearer account-state terminology) without changing the underlying schema or entitlement rules. Phase 06 delivered real, workspace-owned Strategies with nested Setups: a five-table versioned domain, structured Rules with a stable identity across edits, database-enforced immutable locked Versions with complete copy-on-write, archive-only lifecycle, and the real responsive `/app/strategies` management UI in English and Thai. Phase 07 delivered the real `trades`/`mistake_types`/`trade_mistakes`/`trade_rule_checks` schema and a pure, deterministic, exhaustively tested calculation engine (`src/lib/calc/`) covering Planned/Actual/System R, outcome classification, and multi-Trade aggregate/attribution/equity metrics — no UI, service, DAL, Server Action, or write path exists yet, and Discipline Score/mistake-cost attribution remain deliberately unimplemented pending a future evidence-backed decision. See [docs/roadmap.md](docs/roadmap.md#what-phase-07-delivered).
